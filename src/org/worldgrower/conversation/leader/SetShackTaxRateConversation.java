...
public class SetShackTaxRateConversation implements Conversation {
	...
	public List<Question> getQuestionPhrases(WorldObject performer, WorldObject target, HistoryItem questionHistoryItem, WorldObject subjectWorldObject, World world) {
		...
		for(int newShackTaxRate : shackTaxRates) {
			if (newShackTaxRate != currentShackTaxRate) {
				questions.add(new Question(null, "I want to change the shack tax rate from " + currentShackTaxRate + " to " + newShackTaxRate + " gold pieces per 100 turns", newShackTaxRate));
			}
		}
	...
	public void handleResponse(int replyIndex, ConversationContext conversationContext) {
		...
		if (replyIndex == YES) {
			GroupPropertyUtils.getVillagersOrganization(world).setProperty(Constants.SHACK_TAX_RATE, newShackTaxRate);
		}
	}
	
	@Override
	public String getDescription(WorldObject performer, WorldObject target, World world) {
		return "talking about changing the tax rate for shacks";
	}
}
